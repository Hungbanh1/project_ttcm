      ...
      {{-- {{dd($cat_title->cat_id)}} --}}
        ...
        <div class="section" id="list-product-wp">
            <div class="section-head clearfix">
                <h3 class="section-title fl-left">{{$cat_title}}</h3>
                <div class="filter-wp fl-right">
                    <p class="desc"></p>
                    <div class="form-filter">
                        <p class="desc">Hiển có {{$products->count()}} trên {{$products->count()}} sản phẩm</p>

                        <form method="GET" action="{{route('sort' , $cat_title)}}">
                            {{-- @dd($cat_title) --}}
                            {{-- @csrf --}}
                            <select name="select">
            ...
            <div class="section-detail">
                <ul class="list-item clearfix">
                    ...
                    <li>
                        {{-- {{dd(asset($item->product_thumb))}} --}}
                        <a href="{{route('detail_product' , ["$item->cat_title","$item->slug"])}}" title="" class="thumb">

                            <img src="{{asset($item->product_thumb)}}" style="width:191px;height:191px">
                            {{-- <img src="{{$item->product_thumb}}" style="width:191px;height:191px"> --}}
                        </a>
